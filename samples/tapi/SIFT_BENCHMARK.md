...
Use an OpenCV tree that contains `samples/tapi/sift_benchmark.cpp` (this benchmark lives under **samples/tapi**). Checkout the branch you want to measure (e.g. `4.x`).
...
### 4. Run with a known library path (Unix install tree)
...
### 5. Benchmark invocation (repeatable matrix)
...
### 6. OpenCL device selection (optional)
...
### 7. SIFT-specific OpenCL toggles (optional)
...
### 8. Sanity checks before trusting numbers
